...
public class Slicable : MonoBehaviour, ITriangleSeparator
{
    ...
    [SerializeField]
    private bool oneSliceOnly = true;
    ...
    {
        if (oneSliceOnly && sliced) return;

        sliced = true;

        if (other.CompareTag("Slicer"))
        {
            CompleteObserver.Instance.HitTarget();
            ...
            if (_slicer.divideMesh(_myMesh, out chunks, out copy))
            {
                ...
                for (int w = 0; w < 2; w++)
                    foreach (Chunk chunk in chunks[w])
                    {
                        ...
                        if (oneSliceOnly)
                        {
                            Destroy(go.GetComponent<Slicable>());
                        }
                        ...
                        //parentRB.AddForce(Vector3.up * 5f, ForceMode.VelocityChange);
                        if (parentRB != null)
                            go.GetComponent<Rigidbody>().AddForce(Vector3.up * 5f, ForceMode.VelocityChange);
                            go.GetComponent<Rigidbody>().AddTorque(Random.onUnitSphere * 10f, ForceMode.VelocityChange);
                        s3.Stop();
                    }
